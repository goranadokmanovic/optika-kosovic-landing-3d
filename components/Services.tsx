...
export default function Services() {
  return (
    <section id="usluge" className="luxury-section bg-luxury-cream py-20 md:py-44">
          ...
          {services.map((service, index) => {
            ...
            return (
              <Reveal key={service.title} delay={index * 0.07}>
                <article className="luxury-card group relative min-h-64 overflow-hidden p-6 md:min-h-72 md:p-7">
                  <div className="absolute inset-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100">
                    <div className="h-full w-full bg-gradient-to-br from-brand-soft via-accent/5 to-transparent" />
                  </div>
                  <div className="relative z-10">
                    <div className="mb-10 inline-flex h-12 w-12 items-center justify-center bg-brand-soft text-accent">
                      <Icon size={21} strokeWidth={1.7} />
                    </div>
                    <h3 className="font-serif text-3xl italic tracking-[-0.04em] text-neutral-950">{service.title}</h3>
